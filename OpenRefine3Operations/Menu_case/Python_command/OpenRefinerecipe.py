...
def edit(project_id,column,clusterer_type='binning',function='ngram-fingerprint', params=20):
    computeCluster=compute_clusters(project_id,column,clusterer_type,function,params)
    edit_from=getFromValue(computeCluster)
    ### still have something to imporve
    edit_to=getToValue(computeCluster)
    edits=[{'from':f, 'to':t} for f,t in zip(edit_from, edit_to)]
    return edits


def mass_edit(project_id,column,clusterer_type,function,params,expression='value'):
    edits=edit(project_id,column,clusterer_type,function,params)
...
def main():
    create_project('partTest.csv','try6')
