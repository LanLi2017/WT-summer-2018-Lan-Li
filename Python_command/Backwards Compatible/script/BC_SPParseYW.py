...
def main():
    with open('../OriginalOR.json','r')as f:
        data=json.load(f)
    ...
    # parse into YW model
    f=open('../yw/BC_SPParseYW.txt','w')
    f.write('#@begin BC_SPOR2YW#@desc Workflow of Linear original openrefine history\n')
        ...
        if splitlists[a][0]['op'] == 'core/column-split':
            ...
            for b in range(1, innerlenth):
                ...
                if opname == 'core/mass-edit':
                    ...
                    try:
                        f.write('#@param cluster-type:%s\n'%splitlists[a][b]['Cluster-type'])
                        f.write('#@param cluster-params:%s\n'%splitlists[a][b]['Cluster-params'])
                        f.write('#@param cluster-function:%s\n'%splitlists[a][b]['Cluster-function'])
                    except KeyError:
                        f.write('#@param Edits:%s\n'%str(splitlists[a][b]['edits']))
